...
Демо-класс `ZCL_XTT_DEMO_120` передаёт объект непосредственно в `MERGE( )`. Такой вариант подходит, когда класс уже содержит необходимые поля и таблицы, а отдельная корневая структура лишь дублировала бы данные.
