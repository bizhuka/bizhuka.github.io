...
Демо-класс `ZCL_XTT_DEMO_030` показывает вложенное повторение: таблицу корневых записей, каждая из которых содержит таблицу деталей. Границы блоков должны быть однозначными, чтобы XTT правильно определил копируемый диапазон на каждом уровне.

## Структура данных и границы
