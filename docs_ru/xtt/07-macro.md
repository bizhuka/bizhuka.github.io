...
Демо-класс `ZCL_XTT_DEMO_070` описывает вызов VBA и изменение сформированного XML через `PREPARE_RAW`. Предпочтителен `PREPARE_RAW`: VBA зависит от настольного Excel, SAP GUI, OLE, настроек безопасности и интерактивного сеанса, поэтому не подходит для фоновой и веб-обработки.
