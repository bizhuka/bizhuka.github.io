...
Демо-класс `ZCL_XTT_DEMO_160` вызывает метод объекта из маркера. Используйте `;call=`, если отображаемое значение слишком сложно для короткого [`;cond=`](../cond/), но не требует отдельного поля в контексте отчёта. Длинную или повторяемую логику представления лучше вынести в метод.
