...
Демо-класс `ZCL_XTT_DEMO_140` выводит или удаляет целую область шаблона по ABAP-условию. Используйте `;type=block`, когда условие относится к диапазону строк или PDF-подформуляру, а не к одному значению.
