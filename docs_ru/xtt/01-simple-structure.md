...
Демо-класс `ZCL_XTT_DEMO_010` - минимальный законченный пример XTT. Он передаёт одну ABAP-структуру в шаблон и формирует XLSX, HTML, DOCX или PDF. Компоненты структуры соответствуют маркерам шаблона; тот же принцип применяется к атрибутам объекта.
