...
Демо-класс `ZCL_XTT_DEMO_040` описывает скалярные типы и постобработку XLSX. Формулы, сортировка и форматирование Excel зависят от фактического типа ячейки, поэтому ABAP-типы следует сохранять и переопределять только по явному требованию шаблона.

## Типы и преобразования

Тип данных критичен для Excel, поскольку от него зависят формулы, сортировка и форматирование ячейки.
...
Первые четыре типа определяются автоматически. `boolean` и `datetime` указываются явно.
...
Для переноса строки используйте `CL_ABAP_CHAR_UTILITIES=>CR_LF`; XTT подставит разделитель целевого формата автоматически.
...
Во внутреннем представлении Excel XML используются относительные ссылки, поэтому специальный знак `$` для XLSX там не требуется.
