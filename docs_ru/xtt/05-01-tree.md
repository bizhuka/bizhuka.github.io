...
Демо-класс `ZCL_XTT_DEMO_050` строит дерево по полям группировки методом `TREE_CREATE`. Используйте его для промежуточных итогов, заголовков групп и отдельных макетов разных уровней; для связи родитель-потомок предназначен [`TREE_CREATE_RELAT`](../tree-group-by-field-relations/).
