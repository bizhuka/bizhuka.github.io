...
Демо-класс `ZCL_XTT_DEMO_090` формирует обе размерности XLSX-таблицы: переменные столбцы и строки данных. Порядок важен - сначала объедините описания столбцов под корнем `C`, затем основные данные под корнем `R`.
