...
Демо-класс `ZCL_XTT_DEMO_110` принимает двоичные данные `XSTRING`, а директива `;type=image` создаёт оболочку `ZCL_XTT_IMAGE` внутри библиотеки. В контексте отчёта остаются только данные, а размеры и формат можно задать в шаблоне.
