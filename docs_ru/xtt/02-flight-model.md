...
Демо-класс `ZCL_XTT_DEMO_022` рассматривает статическое и динамическое объединение ячеек. Директива `;merge=X` объединяет соседние одинаковые значения после расширения данных, когда конечный диапазон заранее неизвестен.
...
Если диапазон растёт вместе с данными и его размер заранее неизвестен, используйте `;merge=X`.

![image](https://user-images.githubusercontent.com/36256417/108017878-3be4cb80-7040-11eb-980a-68c427820b02.png)

При обычном выводе XTT объединяет ячейки вдоль строк данных. Для [`;direction=column`](../output-direction/) объединение выполняется вдоль сформированных столбцов.


***

Объединяются только соседние ячейки с одинаковыми значениями.
...
При неожиданном результате сначала проверьте значения в сформированном отчёте и наличие директивы у нужного маркера.
