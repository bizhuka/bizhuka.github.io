...
Демо-класс `ZCL_XTT_DEMO_021` показывает, как формулы сохраняются при расширении строк и столбцов. Формулы следует создавать в Excel, где их можно проверить; формирование строк формул в ABAP усложняет поддержку и по-разному работает с абсолютными ссылками XLSX и относительными ссылками Excel XML.
...
Используйте следующие правила при редактировании формул непосредственно в **Microsoft Excel**.
...
Для относительных ссылок используйте `OFFSET()` (`СМЕЩ()`) вместе с `INDIRECT()` (`ДВССЫЛ()`), либо нотацию R1C1 через `INDIRECT()`.

Чтобы сложить три предыдущие ячейки, сместитесь на три столбца влево и верните диапазон шириной в три столбца.
...
### Знак $ для строки
Для более простых формул XTT поддерживает специальное правило со знаком `$`:
...
Актуальная версия сохраняет повторяющиеся общие формулы при преобразовании абсолютных и относительных ссылок. Это устраняет потерю формул, характерную для старых выпусков.
