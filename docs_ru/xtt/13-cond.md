...
Демо-класс `ZCL_XTT_DEMO_130` вычисляет небольшие ABAP-выражения внутри маркера. Используйте `;cond=` для логики представления, которая возвращает одно значение. Бизнес-правила и доступ к данным оставляйте в ABAP-классах; короткий шаблон легче читать, тестировать и сопровождать.
