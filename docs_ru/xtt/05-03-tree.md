...
Демо-класс `ZCL_XTT_DEMO_052` рассчитывает значения родителя по дочерним строкам. Для нестандартной логики используйте событие `PREPARE_TREE`; для обычных итогов - функции `SUM`, `AVG`, `COUNT` и `FIRST` в директиве `;func=`.
